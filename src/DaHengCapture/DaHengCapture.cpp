...
DaHengCamera::DaHengCamera(){
    ...
    openParam0.accessMode = GX_ACCESS_EXCLUSIVE;
    openParam0.openMode = GX_OPEN_INDEX;
    openParam0.pszContent = "2";
    //初始化库1
    status = GXInitLib();
    //检测初始化是否成功
...
int DaHengCamera::StartDevice0(int serial_number){
    uint32_t nDeviceNum = 0;
    //枚 举 设 备 列 表
    status0 = GXUpdateDeviceList(&nDeviceNum, 1000);
    if(serial_number>nDeviceNum){
        cout<<"设备号2错误，超过所枚举数量"<<endl;
        return -1;
    }
    //打 开 设 备
    status0 = GXOpenDevice(&openParam0, &hDevice0);
    if(status0 == GX_STATUS_SUCCESS){
        cout<<"设备2打开成功!"<<endl;
        return nDeviceNum;
    }else{
...
/**
 * @brief DaHengCamera::SetTrigger0 设置设备触发类型，设置开始采集图像之前
 * @param uint8_t i (1 触发 0非触发)
 * @return bool 返回是否设置成功
 */
bool DaHengCamera::SetTrigger(uint8_t i){
    ...
    status=GXSetEnum(hDevice,GX_ENUM_ACQUISITION_MODE,GX_ACQ_MODE_MULITI_FRAME);
    if(status == GX_STATUS_SUCCESS){
        cout<<"触发设置1成功!"<<endl;
    }else{
        cout<<"触发设置1失败!"<<endl;
...
bool DaHengCamera::SetTrigger0(uint8_t i){
    ...
    status0=GXSetEnum(hDevice0,GX_ENUM_TRIGGER_ACTIVATION,GX_TRIGGER_ACTIVATION_RISINGEDGE);
    if(status0 == GX_STATUS_SUCCESS){
        cout<<"触发设置2成功!"<<endl;
    }else{
        cout<<"触发设置2失败!"<<endl;
...
bool DaHengCamera::SetStreamOn(){
    ...
    if(status == GX_STATUS_SUCCESS){
        cout<<"buffer1设置成功!"<<endl;
    }else{
        cout<<"buffer1设置失败!"<<endl;
    }
    status0 = GXSetAcqusitionBufferNumber(hDevice0,2);
    if(status0 == GX_STATUS_SUCCESS){
        cout<<"buffer2设置成功!"<<endl;
    }else{
        cout<<"buffer2设置失败!"<<endl;
    }
    //开 采
...
//           Src.copyTo(img);
          ...
          cout<<"相机1读取图片缓冲失败"<<endl;
          return false;
      }
}
bool DaHengCamera::GetMat0(Mat &Src0){
...
//    GXFlushQueue(hDevice);
    ...
    int64_t nPayLoadSize = 0;
    //获 取 图 像 buffer 大 小 , 下 面 动 态 申 请 内 存
    status0 = GXGetInt(hDevice0, GX_INT_PAYLOAD_SIZE, &nPayLoadSize);
    if (status0 == GX_STATUS_SUCCESS&&nPayLoadSize> 0)
    {
        //定 义 GXGetImage 的 传 入 参 数
        GX_FRAME_DATA stFrameData;
        //根 据 获 取 的 图 像 buffer 大 小 m_nPayLoadSize 申 请 buffer
        ...
        //发 送 开 始 采 集 命 令
        status0 = GXSendCommand(hDevice0, GX_COMMAND_ACQUISITION_START);
        if (status0 == GX_STATUS_SUCCESS)
        {
            //调 用 GXGetImage 取 一 帧 图 像
            status0 = GXGetImage(hDevice0, &stFrameData, 100);
//            time0=((double)getTickCount()-time0)/getTickFrequency();
//           printf("相机2采集图像 %gms\n",time0*1000);
    ...
    GXFlushQueue(hDevice);
    //调 用 GXDQBuf 取 一 帧 图 像
    status0 = GXDQBuf(hDevice0, &pFrameBuffer0, 1000);
      if(status0 == GX_STATUS_SUCCESS&&pFrameBuffer0->nStatus == GX_FRAME_STATUS_SUCCESS){
           Src0.create(pFrameBuffer0->nHeight-1,pFrameBuffer0->nWidth-1,CV_8UC3);

           uint8_t *t = (uint8_t *)pFrameBuffer0->pImgBuf;
//           char *pRGB24Buf = new char[pFrameBuffer->nWidth * pFrameBuffer->nHeight * 3];
//           if (pRGB24Buf == NULL)
//           {
//                 return false;
...
//           Src.copyTo(img);
           for(int i = 0;i<Src0.rows;i++){
               Vec3b *p = Src0.ptr<Vec3b>(i);
               for(int j = 0;j<Src0.cols;j++){
                   if(i%2 == 0){
                       if(j%2 == 0){
                           p[j][0] = t[j+1+(i+1)*pFrameBuffer0->nWidth];
                           p[j][1] = (t[j+1+i*pFrameBuffer0->nWidth] + t[j+(i+1)*pFrameBuffer0->nWidth])/2;
                           p[j][2] = t[j+i*pFrameBuffer0->nWidth];
                       }else{
                           p[j][0] = t[j+(i+1)*pFrameBuffer0->nWidth];
                           p[j][1] = (t[j+1+(i+1)*pFrameBuffer0->nWidth] + t[j+i*pFrameBuffer0->nWidth])/2;
                           p[j][2] = t[j+1+i*pFrameBuffer0->nWidth];
                       }
                   }else{
                       if(j%2 == 0){
                           p[j][0] = t[j+1+i*pFrameBuffer0->nWidth];
                           p[j][1] = (t[j+1+(i+1)*pFrameBuffer0->nWidth] + t[j+i*pFrameBuffer0->nWidth])/2;
                           p[j][2] = t[j+(i+1)*pFrameBuffer0->nWidth];
                       }else{
                           p[j][0] = t[j+i*pFrameBuffer0->nWidth];
                           p[j][1] = (t[j+(i+1)*pFrameBuffer0->nWidth] + t[j+1+i*pFrameBuffer0->nWidth])/2;
                           p[j][2] = t[j+1+(i+1)*pFrameBuffer0->nWidth];
                       }
                   }
               }
           }
           //调 用 GXQBuf 将 图 像 buf 放 回 库 中 继 续 采 图
           status0 = GXQBuf(hDevice0, pFrameBuffer0);
           return true;
      }else{
          cout<<"相机2读取图片缓冲失败"<<endl;
          return false;
      }
}
...
bool DaHengCamera::SetResolution(int width_scale, int height_scale){
    ...
    if(status == GX_STATUS_SUCCESS){
        cout<<"相机1分辨率设置成功"<<endl;
        return true;
    }else{
        cout<<"相机1分辨率设置失败"<<endl;
        return false;
    }
}
bool DaHengCamera::SetResolution0(int width_scale, int height_scale){
    //配 置 一 个 2x2 的 Binning 和 2x2 的 Decimation
    ...
    //设 置 水 平 和 垂 直 Binning 模 式 为 Sum 模 式
    status0 = GXSetEnum(hDevice0,GX_ENUM_BINNING_HORIZONTAL_MODE,
    GX_BINNING_HORIZONTAL_MODE_AVERAGE);
    status0 = GXSetEnum(hDevice0,GX_ENUM_BINNING_VERTICAL_MODE,
    GX_BINNING_VERTICAL_MODE_AVERAGE);
    status0 = GXSetInt(hDevice0, GX_INT_BINNING_HORIZONTAL, nBinningH);
    status0 = GXSetInt(hDevice0, GX_INT_BINNING_VERTICAL, nBinningV);
    status0 = GXSetInt(hDevice0, GX_INT_DECIMATION_HORIZONTAL, nDecimationH);
    status0 = GXSetInt(hDevice0, GX_INT_DECIMATION_VERTICAL, nDecimationV);
    if(status0 == GX_STATUS_SUCCESS){
        cout<<"相机2分辨率设置成功"<<endl;
        return true;
    }else{
        cout<<"相机2分辨率设置失败"<<endl;
        return false;
    }
}
...
bool DaHengCamera::SetExposureTime(int ExposureTime){
    ...
    if(status == GX_STATUS_SUCCESS){
        cout<<"相机1曝光值设置成功"<<endl;
        return true;
    }else{
        cout<<"相机1曝光值设置失败"<<endl;
        return false;
    }
}
bool DaHengCamera::SetExposureTime0(int ExposureTime){
    //相机2设 置  曝 光 值
    status0 = GXSetFloat(hDevice0, GX_FLOAT_EXPOSURE_TIME, ExposureTime);
    if(status0 == GX_STATUS_SUCCESS){
        cout<<"相机2曝光值设置成功"<<endl;
        return true;
    }else{
        cout<<"相机2曝光值设置失败"<<endl;
        return false;
    }
}
...
bool DaHengCamera::SetGAIN0(int value,int ExpGain){
    if(value == 0){
        //选 择 增 益 通 道 类 型
        status0 = GXSetEnum(hDevice0, GX_ENUM_GAIN_SELECTOR, GX_GAIN_SELECTOR_BLUE);
    }else if(value == 1){
        status0 = GXSetEnum(hDevice0, GX_ENUM_GAIN_SELECTOR, GX_GAIN_SELECTOR_GREEN);
    }else if(value == 2){
        status0 = GXSetEnum(hDevice0, GX_ENUM_GAIN_SELECTOR, GX_GAIN_SELECTOR_RED);
    }else{
        status0 = GXSetEnum(hDevice0, GX_ENUM_GAIN_SELECTOR, GX_GAIN_SELECTOR_ALL);
    }
    //设 置  曝 光 值
    status0 = GXSetFloat(hDevice0, GX_FLOAT_GAIN, ExpGain);
    if(status0 == GX_STATUS_SUCCESS)
        return true;
    else
        return false;
...
bool DaHengCamera::Set_BALANCE_AUTO(int value){
    ...
    if(status == GX_STATUS_SUCCESS){
        cout<<"相机1自动白平衡设置成功"<<endl;
        return true;
    }else{
        cout<<"相机1自动白平衡设置失败"<<endl;
        return false;
    }
}
bool DaHengCamera::Set_BALANCE_AUTO0(int value){
    //设 置 连 续 自 动 白 平 衡
    status0 = GXSetEnum(hDevice0,GX_ENUM_BALANCE_WHITE_AUTO,value);
    if(status0 == GX_STATUS_SUCCESS){
        cout<<"相机2自动白平衡设置成功"<<endl;
        return true;
    }else{
        cout<<"相机2自动白平衡设置失败"<<endl;
        return false;
    }
}
...
bool DaHengCamera::Set_BALANCE0(int value, int value_number){
    if(value == 0){
        //选 择 白 平 衡 通 道
        status0 = GXSetEnum(hDevice0,GX_ENUM_BALANCE_RATIO_SELECTOR,GX_BALANCE_RATIO_SELECTOR_BLUE);
    }else if(value == 1){
        status0 = GXSetEnum(hDevice0,GX_ENUM_BALANCE_RATIO_SELECTOR,GX_BALANCE_RATIO_SELECTOR_GREEN);
    }else{
        status0 = GXSetEnum(hDevice0,GX_ENUM_BALANCE_RATIO_SELECTOR,GX_BALANCE_RATIO_SELECTOR_RED);
    }
    status0 = GXSetFloat(hDevice0, GX_FLOAT_BALANCE_RATIO, (float)value_number/10.0);
    if(status0 == GX_STATUS_SUCCESS)
        return true;
    else
        return false;
...
double DaHengCamera::Get_TIMESTAMP(){
    ...
    status0 = GXGetInt(hDevice0,GX_INT_TIMESTAMP_LATCH_VALUE,&time);
    //更新频率为125000000Hz
...
DaHengCamera::~DaHengCamera(){
    ...
    status0 = GXStreamOff(hDevice0);
    //关闭设备链接
    status = GXCloseDevice(hDevice);
    status0 = GXCloseDevice(hDevice0);
    //释放库
    status = GXCloseLib();
    status0 = GXCloseLib();
    cout<<"相机1析构"<<endl;
